...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.antInplaceCanContainAddedAnt(place):
                place.ant.ant_contained = self
            elif self.addedAntCanContainAntInPlace(place):
    ...
    def addedAntCanContainAntInPlace(self, place):
        return self.can_contain(place.ant)

    def antInplaceCanContainAddedAnt(self, place):
        return place.ant.can_contain(self)
...
def beesExistsAt(current_pos):
    return current_pos.bees


def inspectThePlaceInFront(current_pos):
    return current_pos.entrance
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while existPlacesToInspect(current_pos):
            ...
            place_index += 1
            current_pos = inspectThePlaceInFront(current_pos)

        return None

    def isPlaceInThrowerAntRange(self, place_index):
        return self.lower_bound <= place_index <= self.upper_bound
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        reflected_damage = self.getReflectedDamage(amount)
        for i in range(len(bee_lst) - 1, -1, -1):
            """ we use len(bee_lst) because reduce_health method might mutate the self.place.bees if bee_health <= 0 
            by removing the bee, so we might not go over the whole list if we used len(self.place.bees) instead."""
            self.place.bees[i].reduce_health(reflected_damage)
        super().reduce_health(amount)

    def getReflectedDamage(self, amount):
        return amount + (self.damage if self.health - amount <= 0 else 0)


class WallAnt(Ant):
    """WallAnt provides shield of protection to the ants behind it.
    It also likes eating potatoes."""
    name = "Wall"
    food_cost = 4
    implemented = True
...
class HungryAnt(Ant):
    """HungryAnt is a Hungry Ant who loves to eat a `meal of one bee` every 3 rounds."""
    name = 'Hungry'
    food_cost = 4
    chewing_turns = 3
    implemented = True
    ...
    def action(self, gamestate):
        if self.isStillChewing():
            self.turns_to_chew -= 1
        else:
            if beesExistsAt(self.place):
                self.pickARandomBeeAndEat()
                self.turns_to_chew = self.chewing_turns

    def pickARandomBeeAndEat(self):
        bee = random_bee(self.place.bees)
        bee.reduce_health(bee.health)

    def isStillChewing(self):
        return self.turns_to_chew
...
class ContainerAnt(Ant):
    ...
    def action(self, gamestate):
        if self.ant_contained:
            self.ant_contained.health += self.health
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, health=2, *args, **kargs):
        super().__init__(health, *args, **kargs)
...
class QueenAnt(ScubaThrower):
    ...
    def remove_from(self, place):
        if place.ant.name == 'Queen' or (place.ant.is_container and place.ant.ant_contained.name == "Queen"):
            return None
    ...
    def action(self, gamestate):
        ...
        in her tunnel one time for each Ant only.
        """
        self.doubleAntsDamageBehind(self.place.exit)
        super().action(gamestate)
    ...
    def doubleAntsDamageBehind(self, place):
        if place is None:
            return None
        current_ant = place.ant
        if current_ant and not self.already_doubled.get(current_ant):
            Ant.double(current_ant)
            self.already_doubled[current_ant] = True

        self.checkIfContainsAntAndDoubleThisAntDamageIfNotDoneBefore(current_ant)
        self.doubleAntsDamageBehind(place.exit)

    def checkIfContainsAntAndDoubleThisAntDamageIfNotDoneBefore(self, current_ant):
        if current_ant and current_ant.is_container and current_ant.ant_contained:
            if not self.already_doubled.get(current_ant.ant_contained):
                Ant.double(current_ant.ant_contained)
                self.already_doubled[current_ant.ant_contained] = True
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        bee_lst = gamestate.bees
        for i in range(len(bee_lst) - 1, -1, -1):
            gamestate.bees[i].reduce_health(self.damage)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        self.setTurnsForStickyEffectOnTarget()

        def stickyEffect(gamestate):
            if self.turns == 0 or gamestate.time % 2 == 0:
                Bee.action(target, gamestate)
            else:
                return None
            self.turns -= 1
        target.action = stickyEffect

    def setTurnsForStickyEffectOnTarget(self):
        if self.turns == 0:
            self.turns = 2
        else:
            self.turns = 5
